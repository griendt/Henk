...
def parse_rain_data(rains):
    ...
    avgrain = (
        ...
        / 4
    )  # average rain over 20 minute period
    ...
    return "Het is de komende %d minuten droog, daarna gaat het %s" % (
        drycount * 5,
        descr,
    )
...
def raw_weather_report(weerstation="6275", lat=51.81, lon=5.85):
    ...
    samenvatting = html.unescape(
        root.find("verwachting_vandaag").find("samenvatting").text
    )
...
class Weather(Module):
    ...
    def weather_report(self, weerstation="6275", lat=51.81, lon=5.85):
