...
class AI(BasePlayer):
    def pick_trump(self):
        ...
        for color in range(4):  # we go trough every color and assign a value to them
            ...
            if TEN in values:
                val += 5
            if KING in values and QUEEN in values:
                val += 7
            if val > maxval:
                maxval = val
                picked = color
    ...
    def show_trick(self, cards, round):
        ...
        if (
            p == self.index and highest_card(cards).owner == self.index
            ...
            if (
                ...
                if m.value in (SEVEN, EIGHT, NINE, ACE):  # mate is signing
                    ...
                    if (
                        m.color not in self.mate_prefered_colors
                    ):  # if it was already signed, ignore this
    ...
    def trow_away_card(self):
        ...
        for color in colors:
            ...
            if not filt.has(TEN):
                self.pp(
                    "We don't have a Ten in this color, so it's safe to trow a card away there"
                )
                return sorted(filt)[0]
        ...
        if not played_cards:
            ...
            if self.is_playing:
                ...
                if filt:
                    ...
                    if (
                        KING in filt.values()
                    ):  # king is high and there is glory possibility
