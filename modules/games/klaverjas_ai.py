...
from cards import Card, SEVEN, EIGHT, NINE, TEN, JACK, QUEEN, KING, ACE, pp, Cards, create_deck, colornames, valuenames, highest_card, glory_calculation, \
    card_points, fake_card, cmp
...
class BasePlayer(object):
    ...
    def reset(self):
        ...
        self.unknown_colours = [
                                   list(range(4))
                               ] * 3  # The possible colours every player might have
    ...
    def hand_string(self):
        ...
        for i in range(4):
            s += (
                    colornames[i]
                    + ": "
                    + ", ".join([valuenames[c.value] for c in colors[i]])
                    + "\n"
            )
        return s
    ...
    def player_is_playing(
            self, index
    ):  # returns wether the player index is on the 'playing' team
    ...
    def legal_cards(self, played_cards):
        if not played_cards:
            return self.cards  # first to play, everything is legal
        ...
        if cards:  # we can confess colour
            if color != self.trump:
                return cards  # no trumps so no restrictions
            # must not undertrump
            higher = [t for t in cards if t > highest]
            if higher:
                return higher  # We must overtrump
            return cards
        trumps = self.cards.get_trumps()
        if not trumps:
            return self.cards  # Don't have any trumps so everything is legal
        if not played_trumps and not winning:
            return trumps  # We aren't winning and we have trumps, so we must play one of those
        higher = [t for t in trumps if t > highest]
        if higher and not winning:
            return higher  # We must overtrump
        ...
        return (
            self.cards
        )  # we can't overtrump, but we only have trumps so we need to play them.
...
class AI(BasePlayer):
    ...
    def show_trick(self, cards, round):
        ...
        def remove_all_in_color(index, color, exceptions=[]):
            ...
            for cc in [
                a for a in d if (a.color == color and a.value not in exceptions)
            ]:
                d.remove(cc)
            if not exceptions or not any(
                    c in d.filter_color(color).values() for c in exceptions
            ):
                i = (index - 1 - self.index) % 4
                if color in self.unknown_colours[i]:
                    self.unknown_colours[i].remove(color)
            ...
            if round == 1:
                if color != self.trump:  # starting player didn't play a trump
                    ...
                    if cards[0].value == NINE:
                        ...
                        self.pp(
                            "Mate started with non-high trump, so he probably wants it back"
                        )
        ...
        if (
                self.index in (cards[0].owner, cards[1].owner)
                and highest_card(cards).owner == self.index
        ):  # we are playing and have won
            m = (
                cards[2] if (cards[0].owner == self.index) else cards[3]
            )  # the card of your mate
            if (
                    m.color != color and m.color != self.trump
            ):  # mate didn't confess color and didn't trump
                if (
                        m.color in self.prefered_colors
                        and abs(self.prefered_colors[m.color]) > 1
                ):
                    pass
                else:
        ...
        if h.owner not in (self.index, self.partner) and h.owner in (
                cards[0].owner,
                cards[1].owner,
        ):  # opponent won this round early
            m = (
                cards[2] if (cards[0].owner == h.owner) else cards[3]
            )  # the card of its mate
            if m.color not in (
                    color,
                    self.trump,
            ):  # mate didn't confess color and didn't trump
        ...
        for c in cards:
            if c.owner == self.index:
                continue
            if c.color != color:
                if c.owner == self.partner:
                    self.prefered_colors[color] = -2
                ...
                if c.value == TEN:
                    ...
                    if Card(ACE, c.color) in d:
                        d.remove(Card(ACE, c.color))
        ...
        if color == self.trump:  # trump asked
            ...
            highest = cards[0]
            for c in cards[1:]:
                if c.color in (highest.color, self.trump) and c > highest:
                    highest = c
                        ...
                        if c.color != self.trump:  # he didn't trump in
                            if highest.color != self.trump:
                                ...
                                rem = [
                                    a
                                    for a in d
                                    if (a.color == self.trump and a > highest)
                                ]
                                if rem == d.filter_color(self.trump):
                                    remove_all_in_color(c.owner, self.trump)
                                else:
        ...
        if highest.owner != cards[3].owner:  # last person didn't win the round
            if highest.owner != cards[1].owner:  # his mate also didn't
                if cards[3].color == color:  # he confessed color
                    if (
                            glory > 20
                    ):  # at least 40 glory, it would always have been better to prevent it
                        remove_all_in_color(cards[3].owner, color)
                    if (
                            glory == 20 and color != self.trump
                    ):  # he might have chosen not to play the TEN
        ...
        if color != self.trump:
            ...
            if a in self.mystery_cards:  # we haven't seen the ACE yet
                if (
                        a not in cards and cards[0].value != TEN
                ):  # it is not played in this round
                    if (
                            self.trump not in Cards(cards).colors()
                    ):  # not trumped in anywhere
                        pass
            ...
            for i in range(0, 3):
                if cards[i] == a:  # ACE has been played
                    if i == 0 and cards[1] == t:
                        remove_all_in_color(cards[1].owner, color)
                    j = 3 if i == 0 else i + 1
                    if (
                            cards[j] == t and highest_card(cards, self.trump) == a
                    ):  # check all the possibilities for why he would have trown a 10
                        values = Cards(cards).filter_color(color).values()
                        if QUEEN in values:
                            if KING in values:
                                remove_all_in_color(cards[3].owner, color, [JACK])
                            else:
                                remove_all_in_color(cards[3].owner, color, [KING])
                        elif KING in values:
                            remove_all_in_color(cards[3].owner, color, [QUEEN])
        ...
        for i, c in enumerate(cards[1:]):
            ...
            if (
                    c.color != color
                    and c.value in (TEN, ACE)
                    and c != highest
                    and self.index_to_mate(c.owner) != highest.owner
            ):
                mate = (c.owner + 1 - self.index) % 4
                if (
                        i == 2
                        or (mate != 3 and color not in self.unknown_colours[mate])
                        or (color not in self.cards.colors())
                ):
                    self.pp(
                        "Person threw away valuable card to opponent, must not have lower cards to play"
                    )
                    for col in range(4):
                        if col == self.trump:
                            continue
    ...
    def will_win_this_round(self, cards_played):
        ...
        if h.owner != self.partner:
            return False
        if len(cards_played) == 3:
            return True  # Only us left to play
        ...
        if is_trumped:  # winning card is trump
            if d.higher_then(h):
                return False  # Person could overtrump
            else:
                return True
        if d.filter_color(self.trump):
            return False  # Person could trump in
        if d.higher_then(h):
            return False
        return True
            ...
            if len(high_cards) >= len(self.mystery_cards.get_trumps()):
                ...
                if m <= 1 and n <= 1:
                    if high_cards:
                        return high_cards[
                            0
                        ]  # the other team only have a maximum of 1 trump each
                    return self.cards[0]
        ...
        if not high_cards:
            ...
            if not cols and len(self.prefered_colors) == 2:  # two negative suggestions
                cols = [
                    i
                    for i in range(4)
                    if i != self.trump and i not in self.prefered_colors
                ]
            for color in cols:
            ...
            if self.unknown_cards[0].get_trumps():
                colors = list(range(4))
                for color in self.unknown_cards[0].colors():
                    colors.remove(color)
                for color in colors:
                    poss = sorted(
                        [
                            c
                            for c in non_trumps.filter_color(color)
                            if c.value not in (TEN, ACE)
                        ]
                    )
                    if poss:
                        ...
                        return poss[0]
            self.pp(
                "Can't play a signed color, and no TEN to free. Play something low and safe"
            )
            filt = [
                c
                for c in non_trumps
                if not self.glory_possibility(c)
                   and (
                           c.color not in self.prefered_colors
                           or self.prefered_colors[c.color] > 0
                   )
            ]
            if not filt:  # there is always a glory chance
                filt = [
                    c
                    for c in non_trumps
                    if c.color not in self.prefered_colors
                       or self.prefered_colors[c.color] > 0
                ]
                if not filt:
                    filt = non_trumps
    ...
    def sign_mate(self):
        ...
        for color in range(4):
            ...
            if (
                    len(filt) == 1
                    and filt.has(TEN)
                    and not self.is_high(Card(TEN, color))
                    and self.will_win_this_round(played_cards)
            ):
                self.pp(
                    "We play our naked 10 to our mate who is guaranteed to win this round."
                )
                if color not in self.signed_colors:
        ...
        if val == 5:
            ...
            if val == 4:
                self.pp(
                    "We have the ace, a ten, and a small one of a color. Play the small one"
                )
            elif val == 3:
                self.pp("Signal ACE, by playing small card")
        ...
        for color in colors:
            ...
            if (
                    len(filt) == 1
                    and len(self.mystery_cards.filter_color(color)) > 5
                    and filt[0].value in (JACK, QUEEN, KING)
            ):
    ...
    def trow_away_card(self):
        ...
        if len(colors) == 1:
            ...
            c, glory = self.maxmin_glory(
                self.played_cards, deck=poss, maximize=False, color=colors[0]
            )
            return c
        if len(colors) == 0:
        ...
        for color in colors:
            ...
            if (
                    len(filt) == 1
                    and filt[0].value not in (ACE, TEN, SEVEN)
                    and len(self.mystery_cards.filter_color(color)) >= 6
            ):
            ...
            #     return c
            if filt.has(TEN) and not self.is_high(filt.has(TEN)):
                continue
    ...
    def play_card(self, rnum, played_cards):
        ...
        if not played_cards:
            ...
            if (
                    n != 0 and m != 0 and totaltrumps > 1 and trumps and not mate_trumps
            ):  # we want to trade 2 for 1
                if (
                        len(trumps) > 1 or not trumps.has(NINE)
                ):  # More than one trump, or otherwise we have something lower than the nine
                    if high_trumps:
                        self.pp(
                            "Mate doesn't have trump, so we try to draw out 2 trumps of the opponent"
                        )
                    ...
                    options = [
                        ...
                        for c in trumps
                        if not self.glory_possibility(c)
                           and c.value not in (ACE, TEN, NINE)
                    ]
                    if options:
                        self.pp(
                            "Mate doesn't have trump, so we try to draw out 2 trumps of the opponent"
            ...
            if self.is_playing:
                ...
                if n + m != 0:
                    if high_trumps and (
                            len(trumps) > 1 or (not mate_trumps and n > 0 and m > 0)
                    ):
                    ...
                    if len(trumps) > 1:
                        filt = [
                            c
                            for c in trumps
                            if not self.glory_possibility(c) and c.value != NINE
                        ]
                        if filt:
                            self.pp(
                                "Don't have high trump, play low one with no chance of glory"
                            )
                            return self.play_this_card(sorted(filt)[0])
                        self.pp(
                            "Don't have high trump, play lowest trump and hope for the best"
                        )
                        return self.play_this_card(sorted(trumps)[0])
                    if trumps and self.flag_partner_started_with_trump:
                        self.flag_partner_started_with_trump = False
                        self.pp(
                            "Mate started out with non-JACK trump, play trump back to him"
                        )
                        poss = [c for c in trumps if self.glory_possibility(c)]
                        if poss:
                            return self.play_this_card(poss[0])
                        else:
                            return self.play_this_card(sorted(trumps)[0])
                    if trumps:
                        self.pp("No good trump play, playing non-trump")
                    else:
                        self.pp("We have no trumps")
            ...
            if (
                    color != self.trump
                    and highest.value == TEN
                    and played_cards[0].value == TEN
            ):  # a person came out with a 10, so he has the ace
                self.player_has_card(played_cards[0].owner, Card(ACE, color))
            if (
                    color == self.trump
                    and highest.value == NINE
                    and played_cards[0].value == NINE
            ):
            ...
            if not possibilities:
                ...
                if trumps:
                    opponent_trumps = max(
                        (
                            len(self.unknown_cards[0].get_trumps()),
                    ...
                    if opponent_trumps < len(high_trumps):
                        ...
                        if non_trumps == high_non_trumps or (
                                not self.will_win_this_round(played_cards)
                                and high_non_trumps
                                and len(high_trumps) > 1
                        ):
                            if non_trumps == high_non_trumps:
                                self.pp(
                                    "We pretrump because we only have high cards left"
                                )
                            else:
                                self.pp(
                                    "We pretrump because we might lose this round, and we still have some high cards"
                                )
                            options = [
                                c for c in high_trumps if not self.glory_possibility(c)
                            ]
                            if options:
                                return self.play_this_card(options[0])
                    ...
                    if winning:
                        ...
                        if non_trumps:
                            if self.is_high(highest) or self.will_win_this_round(
                                    played_cards
                            ):
                    ...
                    if len(trumps) == 1:
                        ...
                        if not played_trumps:
                            ...
                            if c > h:
                                self.pp(
                                    "We can defeat the highest played trump, so do it"
                                )
                                return self.play_this_card(c)
                            else:
                                if non_trumps:
                        ...
                        if played_trumps:
                            ...
                            return self.play_this_card(
                                trumps[0]
                            )  # we play our lowest trump
                else:
                ...
                if color == self.trump:
                    ...
                    if filt:
                        ...
                        if high_cards and self.is_playing:
                            self.pp(
                                "We play a high trump, maximizing glory possibility"
                            )
                        ...
                        c, glory = self.maxmin_glory(
                            played_cards, deck=filt, maximize=False
                        )
                        return self.play_this_card(c)
                    ...
                    if winning and self.is_high(highest):
                        if NINE not in trumps.values() or ACE in trumps.values():
                            ...
                            c, glory = self.maxmin_glory(
                                played_cards,
                                deck=[a for a in trumps if a.value != NINE],
                            )
                        return self.play_this_card(c)
                    if ACE not in trumps.values():
                        ...
                        c, glory = self.maxmin_glory(
                            played_cards, deck=trumps, maximize=False
                        )
                    return self.play_this_card(c)
                if len(played_cards) == 3:  # we are the last one to play a card
                    if winning:
                        self.pp(
                            "We are currently winning the round, and are last in line"
                        )
                        c, glory = self.maxmin_glory(played_cards)
                        if glory:
                    ...
                    if winning:
                        if self.is_high(highest) or (
                                highest.color == self.trump and color != self.trump
                        ):
                            ...
                            if any(c > highest for c in possibilities):
                                ...
                                if len([c for c in self.cards if self.is_high(c)]) > 1:
                                    self.pp(
                                        "We have high cards so we take over this one"
                                    )
                                    filt = [c for c in possibilities if c > highest]
                                    c, glory = self.maxmin_glory(
                                        played_cards, deck=filt
                                    )
                                    return self.play_this_card(c)
                        ...
                        self.pp(
                            "We are currently winning, but there is a higher card in play, duck"
                        )
                        c, glory = self.maxmin_glory(played_cards, maximize=False)
                        return self.play_this_card(c)
                    filt = Cards([c for c in possibilities if self.is_high(c)]).sorted()
                    if (
                            filt and filt[0] > highest
                    ):  # this last check is because it might be trumped in
                        ...
                        if len(played_cards) == 1:
                            if possibilities.has(TEN):
                                possibilities.remove(Card(TEN, played_cards[0].color))
                        if len(played_cards) == 2 and possibilities.has(TEN):
                            no_glory = Cards(
                                [
                                    c
                                    for c in possibilities
                                    if not glory_calculation(
                                    played_cards + [c], self.trump
                                )
                                ]
                            )
                            if no_glory.has(TEN):
                                if len(no_glory) == 1:
                                    self.pp(
                                        "Only way to dodge glory is trowing away our TEN"
                                    )
                        ...
                        self.pp("we can't win, play a low card")
                        c, glory = self.maxmin_glory(
                            played_cards, maximize=False, deck=possibilities
                        )
                        return self.play_this_card(c)
    ...
    def do_minmax(self, amount=None):
        ...
        if self.played_cards and self.played_cards[0].value == TEN:
            self.player_has_card(
                self.played_cards[0].owner, Card(ACE, self.played_cards[0].color)
            )
        if self.round < 6:
        ...
        for i, d in enumerate(
                self.generate_all_distributions(self.round, self.played_cards)
        ):
        ...
        if count > maxcount:
            ...
            distributions = [
                self.generate_distribution(self.round, self.played_cards)
    ...
    def glory_possibility(self, card):
        ...
        for a in d1:
            for b in d2:
                for c in d3:
                    if glory_calculation([a, b, c], self.trump):
                        continue  # disregard glory caused by other cards
    ...
    def maxmin_glory_prime(self, cards_played, maximize=True, deck=None, color=None):
        ...
        if len(cards) == 2:
            for c in poss:
                other = self.unknown_cards[0].filter_color(
                    c.color
                )  # the cards of the next player
                if not other:
                    other = self.unknown_cards[0].filter_color(cards[0].color)
                if not other:
                    g = glory_calculation(cards + [c], self.trump)
                    if mult * cmp(g, glory) == 1:
                        glory = g
...
class MinMaxer(object):
    def __init__(
            self, players, trump, points1, points2, currentplayer, round, played_cards
    ):
