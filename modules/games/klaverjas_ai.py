...
class BasePlayer:
    ...
    def pp(self, s, index=-1):
        if self.silent:
            return
        if index == -1 or self.index == index:
            self.printer(f'{self.name}: {s}')
...
class AI(BasePlayer):
    ...
    def show_trick(self, cards, round):
        ...
        if self.index in (cards[0].owner, cards[1].owner) and highest_card(cards).owner == self.index:  # we are playing and have won
            ...
            if m.color != color and m.color != self.trump:  # mate didn't confess color and didn't trump
                if m.color in self.prefered_colors and abs(self.prefered_colors[m.color]) > 1:
                    pass
                elif m.value in (SEVEN, EIGHT, NINE, ACE):  # mate is signing
    ...
    def pick_non_trump(self):
        ...
        if not non_trumps:
            ...
            if filt:
                return filt[0]
            if len(high_cards) >= len(self.mystery_cards.get_trumps()) or len(high_cards) > 1:
                return high_cards[0]
            else:
                ...
                if m <= 1 and n <= 1:
                    ...
                    return self.cards[0]
                elif self.round >= 6:  # don't risk it, play the lower 1
        ...
        if not high_cards:
            ...
            if not filt:  # there is always a glory chance
                ...
                if not filt:
                    filt = non_trumps
            lowest = Cards(filt).filter_value(sorted(Cards(filt).values())[0])
            if not lowest:
                    ...
                    if len(filt) > 3:  # We have A 10 x x
                        ...
                        if filt.has(SEVEN) or filt.has(EIGHT) or filt.has(NINE):
                            if val < 4:
                                val = 4
                                col = color
                            elif self.will_win_this_round(played_cards):
                                if val < 3:
                                    val = 3
                                    col = color
            ...
            if not possibilities:
                ...
                if trumps:
                    ...
                    if opponent_trumps < len(high_trumps):
                        ...
                        if non_trumps == high_non_trumps or (not self.will_win_this_round(played_cards) and high_non_trumps and len(high_trumps) > 1):
                            if non_trumps == high_non_trumps:
                                self.pp('We pretrump because we only have high cards left')
                            else:
                                self.pp('We pretrump because we might lose this round, and we still have some high cards')
                ...
                if color == self.trump:
                    ...
                    if ACE not in trumps.values():
                        ...
                        c, glory = self.maxmin_glory(played_cards, deck=trumps, maximize=False)
                    return self.play_this_card(c)
    ...
    def do_minmax(self, amount=None):
        ...
        #     distributions = [d for i,d in enumerate(distributions) if i%skipvalue == 0]
            ...
            self.pp(f'Evaluating {count!s} possibilities')
        ...
        for c, scores_for_card in options.items():
            ...
            self.pp(f'Minmax: {c.pretty()} has average score of {avg:.2f}')
            if avg > best_score:
                best_score = avg
            scores[c] = avg
    ...
    def maxmin_glory(self, cards, maximize=True, deck=None, color=None):
        ...
        if glory - baseglory != 0:
            self.pp(f'Glory calculation: {glory - baseglory:.1f}')
        return (c, glory)
