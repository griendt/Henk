...
def test_trump_choice(seed):
    ...
    for i in range(4):
        print(
            colornames[i] + ": " + ", ".join([valuenames[c.value] for c in colors[i]])
        )
    trump = ai.pick_trump()
...
def performance_test(ai_class1, ai_class2=BaseAI, ngames=1000):
    ...
    for i in range(ngames):
        g = Game(
            silent=2,
            seed=r.randint(10000000, 20000000),
            players=players,
            cancelpoints=False,
        )
        g.play_game()
...
def performance_compare(ai_class1, ai_class2, ai_class3):
    ...
    print(
        "Aanvallend heeft AI 1", game1[2], "keer nat en", game1[3], "punten verschil."
    )
    print(
        "Verdedigend heeft AI 1 de ander",
        game2[2],
        "keer nat gespeeld en",
        game2[3],
        "punten verschil gemaakt.",
    )
    print(
        "Aanvallend heeft AI 2", game3[2], "keer nat en", game3[3], "punten verschil."
    )
    print(
        "Verdedigend heeft AI 2 de ander",
        game4[2],
        "keer nat gespeeld en",
        game4[3],
        "punten verschil gemaakt.\n",
    ...
    if game1[3] > game3[3]:
        print(
            "Aanvallend heeft AI 1",
            game1[3] - game3[3],
            "meer puntenverschil weten te behalen",
        )
    elif game1[3] < game3[3]:
        print(
            "Aanvallend heeft AI 2",
            game3[3] - game1[3],
            "meer puntenverschil weten te behalen",
        )
    elif game1[3] == game3[3]:
        print(
            "Aanvallend hebben AI 1 en AI 2 exact hetzelfde puntenverschil behaald. Wonderlijk."
        )
    ...
    if game2[3] > game4[3]:
        print(
            "Verdedigend heeft AI 1 ",
            game2[3] - game4[3],
            "meer puntenverschil weten te behalen",
        )
    elif game2[3] < game4[3]:
        print(
            "Verdedigend heeft AI 2 ",
            game4[3] - game2[3],
            "meer puntenverschil weten te behalen",
        )
    elif game2[3] == game4[3]:
        print(
            "Verdedigend hebben AI 1 en AI 2 exact hetzelfde puntenverschil behaald. Magistraal."
        )
...
def game_diff(g1, g2):
    ...
    msg = "{} ({!s}) {!s} | {!s} ({!s})    vs  {} ({!s}) {!s} | {!s} ({!s})\n".format(
        suit_to_unicode[g1.trump],
        g1.pointsglory1,
        g1.points1,
        g1.points2,
        g1.pointsglory2,
        suit_to_unicode[g2.trump],
        g2.pointsglory1,
        g2.points1,
        g2.points2,
        g2.pointsglory2,
    ...
    for i in range(len(g1.round_lists)):
        if g1.round_lists[i] == g2.round_lists[i]:
            continue
        else:
            break
...
class Game(object):
    def __init__(
        self, silent=0, seed=None, players=[], cancelpoints=True, startingplayer=0
    ):
        self.should_pause = True if silent < 2 else False
        self.silent = silent
        self.cancelpoints = (
            cancelpoints  # whether points should be added to other team in case of wet
        )
        self.startingplayer = startingplayer
        self.chatter = []
        if not seed:
            self.seed = random.randint(10000000, 20000000)
        else:
            self.seed = seed
        self.players = []
        self.deck = None
    ...
    def do_round(self):
        ...
        for i in range(4):
            ...
            if self.silent < 2:
                pp("{}: {}".format(p.name, cards[-1].pretty()))
