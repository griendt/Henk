...
class Poll(Module):
    ...
    def poll(self, bot, msg):
        ...
        d = text.split("|")
        if len(d) == 1:
            options = ["\u2764", "\U0001f4a9"]  # heart and poop
        elif len(d) > 6:
        ...
        for i, o in enumerate(options):
            buttons.append(
                InlineKeyboardButton(
                    text=o, callback_data="poll%d:%d" % (len(self.polls), i)
                )
            )
        keyboard = InlineKeyboardMarkup(inline_keyboard=[buttons])
        sent = bot.telebot.sendMessage(
            msg.chat_id, "Poll: %s" % query, reply_markup=keyboard
        )
        ident = telepot.message_identifier(sent)
        bot.dataManager.add_poll(
            ident[0], ident[1], len(self.polls), query + "|" + "|".join(options), "{}"
        )
    ...
    def callback(self, bot, msg):
        ...
        option = int(option)
        if (
            str(from_id) in self.pollvotes[poll]
            and self.pollvotes[poll][str(from_id)] == option
        ):
            return
        ...
        bot.dataManager.add_poll(
            ...
            poll,
            p[1] + "|" + "|".join(p[2]),
            json.dumps(self.pollvotes[poll]),
        )
        ...
        for i, o in enumerate(self.polls[poll][2]):
            ...
            if c == 0:
                ...
                s = "%s %d" % (o, c)
            buttons.append(
                InlineKeyboardButton(text=s, callback_data="poll%d:%d" % (poll, i))
            )
        keyboard = InlineKeyboardMarkup(inline_keyboard=[buttons])
        editor.editMessageReplyMarkup(reply_markup=keyboard)
        bot.telebot.answerCallbackQuery(
            query_id, text="Je hebt gestemd op " + p[2][option]
        )
        return
