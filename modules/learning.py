...
class Learning(Module):
    ...
    def learn(self, bot, msg):
        ...
        if c in (
            bot.commands["introductions"]
            + bot.commands["funny"]
            + bot.commands["amuse"]
            + bot.commands["spam_ask"]
        ):
        ...
        if c.startswith("$"):  # special command
            if c[1:] not in bot.responses.keys():
                ...
                bot.responses[c[1:]].extend(responses)
        bot.dataManager.add_response(c, responses, msg.sender, msg.date)
        if not c.startswith("$"):
            if c in bot.aliasdict:
                bot.userresponses[bot.aliasdict[c]].extend(responses)
            else:
                bot.aliasdict[c] = (
                    len(bot.userresponses) + 1
                )  # add new to list of aliases
    ...
    def showalias(self, bot, msg):
        ...
        if not s in l:
            ...
            if not options:
                ...
                return (
                    "Ik ken deze niet, maar het lijkt wel op deze die ik wel ken: \n%s"
                    % "\n".join(options)
                )
        i = bot.aliasdict[s]
        ...
        if len(aliases) == 1:
            ...
            if not options:
                ...
                response += (
                    "Ik ken geen synoniemen van deze term, maar hij lijkt wel veel op deze: \n%s\nIs ie gelijk aan een van deze?"
                    % "\n".join(options)
                )
        else:
