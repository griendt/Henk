...
class Calc(Module):
    ...
    def learnstats(self, bot, msg):
        ...
        return (
            "Ik ken %d custom queries, en heb daar in totaal %d responses op. Verder ken ik %d aliases"
            % (c, d, aa)
        )

    def response_math(
        self, t, bot, clean=False
    ):  # if not clean it outputs random error messages
        try:
            result = simpleeval.simple_eval(
                t.replace("^", "**"), functions=math_functions, names=math_constants
            )
            if type(result) == bool:
                if result:
                    result = "waar"
            ...
            if not clean:
                return bot.pick(bot.responses["math_error"])
            else:
